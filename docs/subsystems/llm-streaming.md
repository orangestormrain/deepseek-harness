...
Adapters whose routes authenticate through OAuth — an account rather than a key — additionally implement the seam's login operations. The interaction surface is structurally the same prompt/event vocabulary pi-ai's OAuth flows drive, so an adapter hands it through without translation; flow steps reach surfaces through the `llm/oauth-event` host event.
