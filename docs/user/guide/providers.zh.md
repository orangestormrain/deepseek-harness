...
仅以 OAuth 认证的提供方——已安装目录中的 Codex（ChatGPT）——靠登录账号而非密钥来配置。选择**添加提供方**，选取 **OpenAI Codex**，点击**登录 OpenAI 账号**。登录页面会自动在浏览器中打开；流程运行期间卡片会显示进度和取消控件，完成后显示已登录的账号。当浏览器流程无法自行完成时，会回退到设备码流程与粘贴授权码两种方式。

如果你已经用官方 Codex CLI 登录过 ChatGPT（`codex login`），该路由会直接采纳这个账号——卡片无需任何流程即可显示已登录。

账号凭据存储在 `$DSH_HOME/.oauth.json`（仅属主可读写），后续请求会自动刷新。点击**退出登录**移除凭据并停止采纳 Codex CLI 的登录；提供方路由本身会保留，直到你删除它。
