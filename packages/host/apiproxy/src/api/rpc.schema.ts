...
export const rpcErrorSchema: z.ZodType<RpcError> = z.discriminatedUnion('code', [
  ...
  z.object({ code: z.literal('session-has-descendants'), message: z.string(), details: z.object({ sessionId: z.string(), descendantSessionIds: z.array(z.string()) }) }),
  ...
  z.object({ code: z.literal('oauth-unknown-provider'), message: z.string(), details: z.object({ provider: z.string() }) }),
  z.object({ code: z.literal('oauth-settings-unavailable'), message: z.string(), details: z.object({ provider: z.string() }) }),
  z.object({ code: z.literal('oauth-provider-inactive'), message: z.string(), details: z.object({ provider: z.string() }) }),
  z.object({ code: z.literal('oauth-no-active-login'), message: z.string(), details: z.object({ provider: z.string() }) }),
  z.object({ code: z.literal('oauth-no-pending-prompt'), message: z.string(), details: z.object({ provider: z.string() }) }),
  z.object({ code: z.literal('oauth-logout-rejected'), message: z.string(), details: z.object({ provider: z.string() }) }),
