...
An installed catalog provider that authenticates through OAuth alone — `openai-codex`, which uses a ChatGPT (Plus/Pro) account against `chatgpt.com/backend-api` — is configured by logging in, not by a key. The Models page offers the route's login card; the web host runs the provider's own flow (browser authorization with a local callback, or a device code), opens the authorization page in the system browser the moment the flow asks for it, relays its steps to the page, and persists the returned credential in the owner-only document `$DSH_HOME/.oauth.json` (one credential per provider route, mode 0600). The route's profile is a reference-free `{}`: requests resolve the credential from that document through the same pi-ai store the login wrote, refreshing expired access tokens under the store lock on first use.

A machine that already signed ChatGPT in through **codex-cli** (`~/.codex/auth.json`, `auth_mode: chatgpt`) is already bound: the route adopts that login on first use — copied into the OAuth document, so the store then owns and refreshes its own copy — and the Models page shows the account without any flow. Signing out blocks the codex source durably (a `null` document entry), so "退出登录" stays off until a real login. Because the adopted copy refreshes independently, alternating between DSH and codex-cli may rotate the shared refresh token; a failed request after such rotation is fixed by signing out and logging in once more. `oauthCodexHome` names the OS home the adoption reads when codex state lives elsewhere.

The login flow is available through the `llm` seam (`login`, `logout`, `oauthStatus`, and the `llm/oauth-event` host event); the harness's OAuth support stops at routes whose adapter runs a flow — a keyless profile on any other route stays unauthenticated, and provider-native ambient discovery still answers a profile that names no credential at all.
...
- **OAuth routes refresh tokens, not logins** — an expired *refresh* token (a revoked ChatGPT account, for instance) fails requests with the provider's own error; the stored credential is kept for diagnosis and re-login replaces it. Nothing watches for revocation out of band.
